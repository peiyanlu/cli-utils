...
export const gitTagDeleteSync = (tag: string, args: string[] = []) => {
 ...
 */
export const getRemoteTags = async (remote = 'origin', sort: GitSort = GitSorter.NAME_DESC) => {
  const res = await gitLsRemote([ '--tags', '--refs', '--sort', sort, remote ])
  return res ? splitLines(res).map(t => t.split('refs/tags/')[1]) : []
}

/** 获取最新 tag */
export const getLatestTag = async (match: string = '*', exclude: string | string[] = '*-beta.*') => {
  const [ latestTag ] = await getLocalTags(match, exclude, GitSorter.NAME_DESC, 1)
  return latestTag
}

/** 获取上一个 tag */
export const getPreviousTag = async (tag: string, match: string = '*', exclude: string | string[] = '*-beta.*') => {
